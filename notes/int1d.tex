...
gives $z_0' = dz_0/d\eta = i/\tH'(z_0)$.
Approaching from the inside means $\re z_0' \overline{z_0}<0$, recalling
that $\eta$ is decreasing to $0$.
...
This has a simple interpretation that only up-going bands should be included
(since

A good check for the sign is the simple test case band structure
$H(k) = \cos k$, whose only nonzero coefficients are $H_{\pm1} = 1/2$,
for which the exact DOS is easily found by change of variable,
$$
\rho(\om) = \int_0^{2\pi} \delta(H(k)-\om) dk
= \sum_{k\in[0,2\pi]: H(k)=\om} \frac{1}{|H'(k)|}
= \frac{2}{\sin(\cos^{-1} \om)} = \frac{2}{\sqrt{1-\om^2}}
$$
in the band $|\om|<1$, or zero when $|\om|>1$.
