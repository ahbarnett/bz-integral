...
This ``Laurent to polynomial'' conversion
trick is due to J.\ Boyd in the context of root-finding.
...
\qquad \mbox{ where }
\quad
\res_{z_j} B^{-1} := \lim_{z\to z_j} (z-z_j)B(z)^{-1} = \frac{v_j u_j^*}{u_j^* B'(z_j) v_j}~,
\label{resmat}
\ee
...
% * use Bindle-Hood winding Rouche to count PEP eigenvalues in disc?
